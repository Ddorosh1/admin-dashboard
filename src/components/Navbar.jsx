...
// Styles for tooltip and navbuttons and wrapping in component
...
const Navbar = () => {
  ...
  // Get client screen size on initial load
  useEffect(() => {
  ...
  // Hide menu if screen size is below certain threshold
  useEffect(() => {
    if(screenSize <= 900) {
      setActiveMenu(false);
    } else {
        ...
        <TooltipComponent content="Profile" position="BottomCenter">
          <div className='flex item-center gap-2 cursor-pointer p-1 hover:bg-light-gray rounded-lg' onClick={() => handleClick('userProfile')}> 
            ...
            <p>
              ...
              <span className="text-gray-400 font-medium ml-1 text-14">Dmitriy</span>
            </p>
            <MdKeyboardArrowDown className="text-gray-400 text-14" />
          </div>
        </TooltipComponent>

        {/* Logic for showing clicked component in navbar */}
        {isClicked.cart && <Cart />}
        {isClicked.chat && <Chat />}
